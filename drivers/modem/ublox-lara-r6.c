...
LOG_MODULE_REGISTER(modem_ublox_lara_r6, CONFIG_MODEM_LOG_LEVEL);
...
#if defined(CONFIG_MODEM_UBLOX_LARA_AUTODETECT_APN)
#include <stdio.h>
#endif
...
#if defined(CONFIG_MODEM_UBLOX_LARA_AUTODETECT_VARIANT)
#define MDM_VARIANT_UBLOX_LARA_R6 4
#define MDM_VARIANT_UBLOX_LARA_R211 2
#endif
...
K_KERNEL_STACK_DEFINE(modem_rx_stack,
		      CONFIG_MODEM_UBLOX_LARA_R6_RX_STACK_SIZE);
struct k_thread modem_rx_thread;

// #if defined(CONFIG_MODEM_UBLOX_LARA_RSSI_WORK)
// /* RX thread work queue */
// K_KERNEL_STACK_DEFINE(modem_workq_stack,
// 		      CONFIG_MODEM_UBLOX_LARA_R6_RX_WORKQ_STACK_SIZE);
...
const char *modem_sim_profiles =
	CONFIG_MODEM_UBLOX_LARA_AUTODETECT_APN_PROFILES;
...
// #if DT_INST_NODE_HAS_PROP(0, mdm_dtr_gpios)
// 	LOG_DBG("MDM_DTR_PIN -> DISABLE");
// 	gpio_pin_set_dt(&dtr_gpio, 0);
// 	k_sleep(K_MSEC(30));
// #endif

	LOG_INF("Bootup sequence");

#if DT_INST_NODE_HAS_PROP(0, mdm_reset_gpios)
	LOG_DBG("MDM_RESET_PIN -> ENABLE");
	gpio_pin_set_dt(&reset_gpio, 1);
	k_sleep(K_SECONDS(1)); // mini 0.05s, max 6s
	...
	LOG_DBG("MDM_POWER_PIN -> ENABLE");
	gpio_pin_set_dt(&power_gpio, 1);
	k_sleep(K_SECONDS(1)); // min 0.15s, max 3.2s
	LOG_DBG("MDM_POWER_PIN -> DISABLE");
	gpio_pin_set_dt(&power_gpio, 0);
	k_sleep(K_SECONDS(6));
...
#if DT_INST_NODE_HAS_PROP(0, mdm_dtr_gpios)
		/* Power saving control: 3, control by UART DTR line */
		SETUP_CMD_NOHANDLE("AT+UPSV=3"),
#endif
...
// 			",2"),
// #endif
		/* Enable UCALLSTAT urc */
		SETUP_CMD_NOHANDLE("AT+UCALLSTAT=1"),
		/* Enable Greeting Text (Emits +UBLXBOOT at startup over uart)*/
		SETUP_CMD_NOHANDLE("AT+CSGT=1,\"+UBLXBOOT\""),
		/* Enable RI interrupt for all URCs */
		SETUP_CMD_NOHANDLE("AT+URING=1"),
		/* Enable CEREG urc */
		SETUP_CMD_NOHANDLE("AT+CEREG=2"),

		// TODO: init GPIOs of the module
		...
		SETUP_CMD_NOHANDLE("AT+CGDCONT=1,\"IP\",\""
					CONFIG_MODEM_UBLOX_LARA_R6_APN "\""),
		/* start functionality */
...
// #endif
	...
	ret = modem_cmd_handler_init(&mctx.cmd_handler, &mdata.cmd_handler_data,
				     &cmd_handler_config);
	if (ret < 0) {
		LOG_ERR("failed to set up modem command handler (%d)", ret);
		goto error;
	}
	...
	if (ret < 0) {
		LOG_ERR("failed to set up modem uart interface (%d)", ret);
		goto error;
	}

	LOG_DBG("command handler and uart init done");
	...
	LOG_DBG("reset gpio set up");
#endif

#if DT_INST_NODE_HAS_PROP(0, mdm_dtr_gpios)
	ret = gpio_pin_configure_dt(&dtr_gpio, GPIO_OUTPUT);
	if (ret < 0) {
		LOG_ERR("Failed to configure %s pin", "dtr");
		goto error;
	}
	LOG_DBG("dtr gpio set up");
#endif
...
// 				  &mdata, NULL,
// 				  CONFIG_MODEM_UBLOX_LARA_R6_INIT_PRIORITY,
// 				  &api_funcs,
// 				  MDM_MAX_DATA_LENGTH);

DEVICE_DT_INST_DEFINE(0, modem_init, NULL, &mdata, NULL,
		      POST_KERNEL, CONFIG_MODEM_UBLOX_LARA_R6_INIT_PRIORITY,
		      NULL);